...
# [重要] 自动更新源
# 已配置为您提供的仓库: https://github.com/lje02/wp-manager
# 注意：文件名必须在仓库中命名为 wp-manager.sh，且位于 main 分支
...
function check_ssl_status() {
    local d=$1; echo -e "${CYAN}>>> [SSL] 正在向 Let's Encrypt 申请证书...${NC}"; for ((i=1; i<=20; i++)); do if docker exec gateway_acme test -f "/etc/nginx/certs/$d.crt"; then echo -e "${GREEN}✔ SSL 证书获取成功: https://$d${NC}"; read -p "按回车返回..."; return 0; fi; echo -n "."; sleep 5; done; echo -e "\n${YELLOW}⚠️ 证书暂未生成 (可能是DNS延迟，请稍后在菜单13检查)${NC}"; read -p "按回车返回...";
}
...
# --- 18. 脚本更新功能 (V51 GitHub集成版) ---
function update_script() {
    clear
    echo -e "${GREEN}=== 脚本自动更新系统 ===${NC}"
    echo -e "当前版本: ${YELLOW}$VERSION${NC}"
    echo -e "更新源仓库: ${CYAN}github.com/lje02/wp-manager${NC}"
    echo "------------------------"
    
    echo -e "${BLUE}>>> 正在连接 GitHub 拉取最新代码...${NC}"
    
    # 下载到临时文件
    temp_file="/tmp/wp_manager_new.sh"
    http_code=$(curl -L -s -w "%{http_code}" -o "$temp_file" "$UPDATE_URL")

    if [ "$http_code" == "200" ] && [ -s "$temp_file" ]; then
        # 简单的完整性检查 (检查是否包含 bash 头)
        if head -n 1 "$temp_file" | grep -q "/bin/bash"; then
            echo -e "${GREEN}✔ 下载成功，正在覆盖安装...${NC}"
            mv "$temp_file" "$0"
            chmod +x "$0"
            echo -e "${GREEN}✔ 更新完成！正在重启脚本...${NC}"
            sleep 1
            exec "$0"
        else
            echo -e "${RED}❌ 下载的文件似乎不是有效的脚本。${NC}"
            echo "请检查 GitHub 仓库中文件是否名为 wp-manager.sh 且内容正确。"
            rm -f "$temp_file"
        fi
    else
        echo -e "${RED}❌ 更新失败 (HTTP $http_code)。${NC}"
        echo "请检查: 1. 服务器能否访问 GitHub; 2. 仓库 main 分支是否存在 wp-manager.sh"
    fi
    read -p "按回车返回..."
}
...
function show_menu() {
    ...
    echo " 16. 整站 备份与还原"
    echo "-----------------------------------------"
    echo -e "${BLUE} u. 检查并更新脚本 (From GitHub)${NC}"
    echo -e "${RED} x. [危险] 彻底卸载脚本与数据${NC}"
    echo " 0. 退出"
    echo "-----------------------------------------"
    echo -n "请选择操作: "
    read option
...
function create_site() {
    ...
    [ -d "$sdir" ] && echo -e "${RED}❌ 目录已存在${NC}" && read -p "..." && return

    echo -e "\n${BLUE}>>> [1/4] 正在创建目录结构...${NC}"
    mkdir -p "$sdir"
    
    echo -e "${BLUE}>>> [2/4] 正在生成 Nginx 与 WAF 配置文件...${NC}"
...
EOF
    ...
    echo -e "${BLUE}>>> [3/4] 正在编排 Docker 容器配置...${NC}"
...
function generate_nginx_conf() {
    ...
    if [ "$mode" == "2" ]; then
        ...
        echo -e "${YELLOW}--- 配置外部资源聚合 (仅高级模式) ---${NC}"; local count=1
        while true; do
...
function create_proxy() {
    ...
    read -p "主目标 URL: " raw_tu; tu=$(normalize_url "$raw_tu"); echo -e "${CYAN}>>> 目标: $tu${NC}"
...
function repair_proxy() {
    ...
    echo -e "${YELLOW}重新配置 Nginx 规则...${NC}"; 
    generate_nginx_conf "$tu" "$d" "$pmode"
        ...
        case $br in
            ...
            2) ls -1 "$SITES_DIR"; echo "----------------"; read -p "输入要还原的域名: " d; s="$SITES_DIR/$d"; backup_root="$s/backups"
                ...
                if [ ! -z "$latest_backup" ]; then
                    echo -e "最新备份: ${CYAN}$latest_backup${NC}"; read -p "使用此备份? (y/n): " use_latest
                    [ "$use_latest" == "y" ] && target_backup="$latest_backup"
                fi
...
while true; do show_menu; case $option in u|U) update_script;; 1) init_gateway "force"; read -p "按回车返回...";; 2) container_ops;; 3) ssh_key_manager;; 4) create_site;; 5) create_proxy;; 6) create_redirect;; 7) list_sites;; 8) delete_site;; 9) change_domain;; 10) repair_proxy;; 11) fix_upload_limit;; 12) manage_firewall;; 13) cert_management;; 14) manage_hotlink;; 15) db_manager;; 16) backup_restore_ops;; x|X|17) uninstall_cluster;; 0) exit 0;; esac; done
